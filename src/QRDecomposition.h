...
{
	const size_t N = Ak.GetN();
    float sum_by_k_col = 0.0;
    for (size_t str = k_step; str < N; ++str)
    {
		sum_by_k_col += Ak.get(str, k_step) * Ak.get(str, k_step);
    }
	auto beta = sign(-Ak.get(k_step, k_step)) * sqrt(sum_by_k_col);
    return beta;
}
...
{
	auto mu = 1 / sqrt(2.0 * beta * beta - 2 * beta * Ak.get(k_step, k_step));
    return mu;
}
...
static TVector<T> count_w(const TMatrix<T>& Ak, int k_step, float beta, float mu)
{
    ...
    {
		w[str] = str < k_step ? 0 : Ak.get(str, k_step) * mu;
    }
